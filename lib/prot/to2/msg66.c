 ...
 *   maxOwnerServiceInfoSz    ;; maximum size service info that Device can
 * receive
 * ]
 */
int32_t msg66(fdo_prot_t *ps)
{
	...
	if (!fdow_start_array(&ps->fdow, 2)) {
		LOG(LOG_ERROR,
		    "TO2.DeviceServiceInfoReady: Failed to start array\n");
		goto err;
	}

	/* Check if REUSE is ON */
	if (fdo_compare_public_keys(ps->owner_public_key, ps->osc->pubkey) &&
	    fdo_compare_byte_arrays(ps->dev_cred->owner_blk->guid,
				    ps->osc->guid) &&
	    fdo_compare_rv_lists(ps->dev_cred->owner_blk->rvlst,
				 ps->osc->rvlst)) {
	...
	if (ps->reuse_enabled && reuse_supported) {
		LOG(LOG_DEBUG,
		    "TO2.DeviceServiceInfoReady: *****Reuse triggered.*****\n");
		// write CBOR NULL
		if (!fdow_null(&ps->fdow)) {
			LOG(LOG_ERROR, "TO2.DeviceServiceInfoReady: Failed to "
				       "write ReplacementHMac\n");
			goto err;
		}
		...
		if (ps->reuse_enabled) {
			LOG(LOG_DEBUG, "TO2.DeviceServiceInfoReady: *****Reuse "
				       "triggered but not supported.*****\n");
		...
		if (resale_supported) {
			...
			LOG(LOG_DEBUG,
			    "TO2.DeviceServiceInfoReady: *****Resale triggered "
			    "but not supported.*****\n");
			// write CBOR NULL
			if (!fdow_null(&ps->fdow)) {
				LOG(LOG_ERROR,
	...
	if (!fdow_unsigned_int(&ps->fdow, ps->maxOwnerServiceInfoSz)) {
		LOG(LOG_ERROR, "TO2.DeviceServiceInfoReady: Failed to write "
	...
	if (ps->maxOwnerServiceInfoSz > MAX_NEGO_MSG_SIZE) {
		LOG(LOG_ERROR,
		    "TO2.DeviceServiceInfoReady: maxOwnerServiceInfoSz can not "
		    "be greater than 65535\n");
		goto err;
	}
	LOG(LOG_DEBUG,
	    "TO2.DeviceServiceInfoReady: Sent maxOwnerServiceInfoSz = %" PRIu64
	    "\n",
	    ps->maxOwnerServiceInfoSz);

	if (!fdow_end_array(&ps->fdow)) {
		LOG(LOG_ERROR,
		    "TO2.DeviceServiceInfoReady: Failed to end array\n");
		goto err;
	}

	/* Encrypt the packet */
	if (!fdo_encrypted_packet_windup(&ps->fdow,
					 FDO_TO2_NEXT_DEVICE_SERVICE_INFO)) {
		LOG(LOG_ERROR, "TO2.DeviceServiceInfoReady: Failed to create "
			       "Encrypted Message\n");
		goto err;
	}
